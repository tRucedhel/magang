...
function areAllVisibleSelectsFilled() {
  const selects = document.querySelectorAll('select[required]');
  return Array.from(selects).every(select => select.offsetParent !== null && select.selectedIndex !== -1);
}
...
function updateButtonState() {
  ...
  if ((isCheckboxChecked && areAllInputsFilledByClass() && areAllVisibleSelectsFilled()) ||
    (areAllVisibleInputsFilled() && areAllVisibleSelectsFilled())) {
